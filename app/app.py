...
def register_node():
    ...
    not_current_node = current_node_url != new_node_url #채우시오 : current_node_url이 new_node_url이 아니면 true(type boolean)
...
def register_nodes_bulk():
    ...
    for network_node_url in all_network_nodes:
        ...
        not_current_node = current_node_url != network_node_url #채우시오 : current_node_url이 new_node_url이 아니면 true(type boolean)
        if node_not_already_present and not_current_node: #두 가지 조건을 모두 만족하면 실행
            #새로운 노드 network_node에 추가
